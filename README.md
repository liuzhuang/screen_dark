...
ScreenDark 是一款 macOS 菜单栏工具，可分别调节内建屏幕和外接显示器亮度。它通过 Gamma 调节让指定显示器变暗，不主动锁定当前会话；只要至少一块显示器处于 `0%` 状态，就会请求 macOS 在系统层面保持所有显示器唤醒，并阻止空闲系统睡眠。

适合在离开工位、继续运行长任务时隐藏屏幕内容，降低后台 AI 程序、Codex、Claude Code 或 Computer Use 因显示器自动关闭或空闲系统睡眠而暂停的风险。

- 分别调节内建屏幕和外接显示器亮度
- 任一显示器为 `0%` 时，在系统层面保持所有显示器唤醒并阻止空闲系统睡眠
- 保存每块显示器变暗前的亮度，点亮时恢复
- 支持为每块显示器设置独立的全局快捷键
- 提供固定的全部点亮快捷键和独立恢复助手
...
- 任一显示器为 `0%` 时，阻止所有显示器因不活跃而自动关闭，并阻止系统空闲睡眠；不阻止屏幕保护程序、主动锁屏、主动睡眠、合盖或关机。
- 外接显示器需要支持 macOS Gamma 调节；部分显示器或连接方式可能不响应。
- ScreenDark 保留当前会话，并在 `0%` 时保持系统级显示唤醒；这为后台 AI 程序和 Computer Use 继续运行保留了必要条件，但实际结果仍取决于屏幕保护程序、主动锁屏、相关程序、系统授权与屏幕捕获方式。
...
4. 任一显示器为 `0%` 时，ScreenDark 会在系统层面保持所有显示器唤醒，并持续阻止空闲系统睡眠。
